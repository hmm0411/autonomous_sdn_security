...
with st.sidebar:
    ...
    st.caption("Trạng thái hiện tại: Đang lấy dữ liệu trực tiếp từ ONOS API")
...
m1.metric("Traffic Status", current_state["level"])
m2.metric("Live Packet Rate", f"{current_state['packet_rate']} pkt/s")
m3.metric("Live Flow Count", current_state["flow_count"])

threat_score = 0 if current_state["level"] == "NORMAL" else (50 if current_state["level"] == "MEDIUM_ATTACK" else 99)
...
# Xử lý CSS dựa trên dữ liệu thật (Volume-based)
...
with col_info:
    ...
    if is_attack:
        ...
        if "Autonomous" in rl_mode:
            ...
            st.caption(f"State: [Packet Rate: {current_state.get('packet_rate')} | Flow: {current_state.get('flow_count')}]")
        else:
    ...
    st.divider()
    st.markdown("#### Real-time Traffic Graph (Packet Rate)")
...
# ==========================================
# 4. ĐỌC DỮ LIỆU TRAINING & LLM
# ==========================================
with st.expander("RL Training Pipeline & Data Flow", expanded=False):
    st.markdown("""
    ### Quy trình xử lý và Huấn luyện (MLOps)
    * **Data Processing:** Dữ liệu chuẩn hóa bằng `MinMaxScaler` trích xuất 8 features.
    * **Offline Training:** Tác nhân DQN & PPO học theo cơ chế Multi-seed (MAX_EPISODES=5000).
    * **Reward Function:** Tối ưu hóa QoS penalty và Security reward.
    """)
    
    log_file = "logs/ppo_training_log.csv" if os.path.exists("logs/ppo_training_log.csv") else ("runs/ppo_training_log.csv" if os.path.exists("runs/ppo_training_log.csv") else None)
    
    if log_file:
        try:
            train_df = pd.read_csv(log_file)
            if 'reward' in train_df.columns:
                st.line_chart(train_df['reward'])
            else:
                st.info("Đã tìm thấy file log nhưng định dạng không khớp biểu đồ.")
        except:
            st.info("Lỗi khi đọc file log training.")
    else:
        st.caption("*(Chưa tìm thấy file log từ logger.py, đang hiển thị biểu đồ mẫu)*")
...
with col1:
    st.header("LLM Cognition Control")
    is_on = os.path.exists(TRIGGER_FILE)
    if is_on:
        st.success("Trạng thái: LLM đang BẬT")
        if st.button("TẮT Lớp Nhận Thức LLM", use_container_width=True):
            if os.path.exists(TRIGGER_FILE):
                os.remove(TRIGGER_FILE)
            st.rerun()
    else:
        st.error("Trạng thái: LLM đang TẮT")
        if st.button("BẬT Lớp Nhận Thức LLM", use_container_width=True):
...
with col2:
    ...
    if st.button("Xóa Log cũ", use_container_width=True):
